...
exports.TerrainEngine = void 0;
exports.createDefaultTerrain = createDefaultTerrain;
...
class TerrainEngine {
    ...
    static materials() { return structuredClone(MATERIALS); }
    material(id) { return MATERIALS[id]; }
    inBounds(x, y) { return x >= 0 && x < this.config.width && y >= 0 && y < this.config.height; }
    get(x, y) { return this.cells.get(key(x, y)); }
    isAir(x, y) { return !this.inBounds(x, y) || !this.cells.has(key(x, y)); }
    setCell(x, y, material, hp, destructible = true) {
        if (!this.inBounds(x, y))
            return;
        const m = MATERIALS[material];
        const cell = { x, y, material, hp: hp ?? m.terrainHP, maxHp: m.terrainHP, destructible, liquid: !!m.liquid, phase: m.liquid ? 'liquid' : 'solid', temperature: m.temperature, wetness: 0, damageVersion: 0 };
    ...
    worldToCell(p) { return { x: Math.floor(p.x / this.config.cellSize), y: Math.floor(p.y / this.config.cellSize) }; }
    cellToWorld(x, y) { return { x: (x + .5) * this.config.cellSize, y: (y + .5) * this.config.cellSize }; }
    queryCircle(center, radius) {
        ...
        for (let y = c.y - r; y <= c.y + r; y++)
            for (let x = c.x - r; x <= c.x + r; x++) {
                ...
                if (!cell)
                    continue;
                if (dist(this.cellToWorld(x, y), center) <= radius + this.config.cellSize * .71)
                    out.push(cell);
            }
        return out;
    }
    ...
    transition(cell, to, cause) {
        ...
        cell.material = to;
        cell.maxHp = m.terrainHP;
        cell.hp = Math.min(cell.hp, m.terrainHP);
        cell.liquid = !!m.liquid;
        cell.phase = m.liquid ? 'liquid' : 'solid';
        cell.temperature = m.temperature;
        cell.damageVersion++;
        this.changed.add(key(cell.x, cell.y));
    ...
    step(dt) {
        if (dt <= 0)
            return;
        this.tick++;
        for (let i = 0; i < this.config.liquidStepsPerTick; i++)
            this.stepLiquids();
        for (let i = 0; i < this.config.collapseStepsPerTick; i++)
            this.stepCollapse();
        this.processReactions();
    }
    stepLiquids() {
        ...
        for (const c of liquids) {
            ...
            if (!below && this.inBounds(c.x, c.y - 1)) {
                this.moveCell(c, c.x, c.y - 1);
                continue;
            }
            if (below && below.material !== 'water' && c.material === 'water' && below.material === 'lava') {
                this.transition(c, 'ice', 'water-lava');
                this.transition(below, 'rock', 'water-lava');
                continue;
            }
            const dirs = Math.random() < .5 ? [-1, 1] : [1, -1];
            for (const dx of dirs) {
                if (!this.inBounds(c.x + dx, c.y))
                    continue;
                if (this.isAir(c.x + dx, c.y)) {
                    this.moveCell(c, c.x + dx, c.y);
                    break;
                }
            }
        }
    ...
    moveCell(c, nx, ny) {
        if (!this.inBounds(nx, ny) || !this.isAir(nx, ny))
            return false;
        const oldX = c.x, oldY = c.y;
        this.cells.delete(key(oldX, oldY));
        c.x = nx;
        c.y = ny;
        this.cells.set(key(nx, ny), c);
        this.changed.add(key(oldX, oldY));
        this.changed.add(key(nx, ny));
        if (c.material === 'water' || c.material === 'lava')
            this.emit({ type: 'liquid_flow', tick: this.tick, x: oldX, y: oldY, toX: nx, toY: ny, material: c.material });
        return true;
    }
    stepCollapse() {
        ...
        for (const c of candidates) {
            ...
            if (neighbors >= this.config.supportMinNeighbors && c.y > this.config.collapseDepth)
                continue;
    ...
    processReactions() {
        ...
        while (this.reactionQueue.length && processed < this.config.chainReactionLimit) {
            ...
            processed++;
            if (r.depth >= this.config.collapseDepth)
                continue;
            ...
            for (const n of this.neighbors(r.x, r.y)) {
                if (n.material === 'wood' && MATERIALS.wood.flammability > .5) {
                    n.temperature += 80;
                    if (n.temperature > 260)
                        this.destroyCell(n, `chain:${r.source}`);
                }
                if (n.material === 'sand' && r.source.includes('explosive'))
                    this.damageCell(n, 8, `chain:${r.source}`, 'explosive');
            }
        }
    ...
    consumeChangedCells() { const out = [...this.changed].map(k => this.cells.get(k)).filter(Boolean); this.changed.clear(); return out; }
    ...
    queryTerrainCircle(center, radius) { return this.queryCircle(center, radius); }
    applyTerrainDamage(cell, amount, source) {
        const current = this.get(cell.x, cell.y);
        if (current)
            this.damageCell(current, amount, source, 'generic');
    }
}
exports.TerrainEngine = TerrainEngine;
function createDefaultTerrain(width, height, cellSize = 16) {
    ...
    for (let x = 0; x < width; x++) {
        for (let y = 0; y < surface; y++) {
            ...
            const m = depth <= 1 ? 'grass' : depth <= Math.max(3, surface * .18) ? 'earth' : depth <= Math.max(5, surface * .45) ? 'stone' : 'rock';
            t.setCell(x, y, m);
        }
    }
    return t;
...
function validateTerrain(t) {
    const errors = [];
    if (t.config.width < 1 || t.config.height < 1)
        errors.push('invalid dimensions');
    for (const c of t.cells.values()) {
        if (c.hp < 0)
            errors.push(`${c.x},${c.y}: hp<0`);
        if (c.maxHp <= 0)
            errors.push(`${c.x},${c.y}: maxHp<=0`);
        if (!MATERIALS[c.material])
            errors.push(`${c.x},${c.y}: unknown material`);
    }
